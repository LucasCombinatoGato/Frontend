...
function criarTarefa() {
  ...
  if (texto == "") {
    return;
  } //Não precisa do ELSE, vai continuar o código se o texto for diferente de "" [vazio]
  // Se não estiver vazia...
  let li = document.createElement("li"); // Criação de uma List Item
  li.innerHTML = texto+ '<button onclick="removerTarefa(this)" class="btnRemove">Remover</button>';

  //Adicionar o li => ul
  let ul = document.getElementById("lista");
  ul.appendChild(li); //Adicionado o li a ul via hierarquia
  // Limpa o campo do input
  tarefa.value = "";
}

function removerTarefa(botao) {
  botao.parentElement.remove(); //Remove a terefa da lista
}
